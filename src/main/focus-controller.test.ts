...
test("resizing the bar saves its size with its corner, and impossible or stale sizes are dropped", async (context) => {
  const f = await fixture(context);
  const goalId = readyToStart(f);
  f.controller.start({ goalId, intention: "", durationMinutes: 25 });
  assert.equal(f.bar.snapshots.at(-1)?.width, 460, "a bar that has never been resized is the compact one");
  assert.equal(f.bar.snapshots.at(-1)?.height, 54);
  const clockBefore = f.bar.snapshots.at(-1)?.endsAtEpochSeconds;

  for (const line of [
    JSON.stringify({ action: "resized", sessionId: "session-fixed", x: 0, y: 40 }),
    JSON.stringify({ action: "resized", sessionId: "session-fixed", x: 0, y: 40, width: 1_200 }),
    JSON.stringify({ action: "resized", sessionId: "session-fixed", x: 0, y: 40, height: 96 }),
    JSON.stringify({ action: "resized", sessionId: "session-fixed", x: 0, y: 40, width: 10, height: 96 }),
    JSON.stringify({ action: "resized", sessionId: "session-fixed", x: 0, y: 40, width: 100_000, height: 96 }),
    JSON.stringify({ action: "resized", sessionId: "session-fixed", x: 0, y: 40, width: 1_200, height: 10 }),
    JSON.stringify({ action: "resized", sessionId: "session-fixed", x: 0, y: 40, width: 1_200, height: 100_000 }),
    JSON.stringify({
      action: "resized", sessionId: "session-fixed", x: Number.MAX_VALUE, y: 40, width: 1_200, height: 96
    }),
    JSON.stringify({ action: "resized", sessionId: "session-other", x: 0, y: 40, width: 1_200, height: 96 })
  ]) {
    f.bar.handler!(line);
  }
  assert.equal(f.bar.snapshots.at(-1)?.width, 460, "a missing, impossible or stale resize changes nothing");
  assert.equal(f.bar.snapshots.at(-1)?.height, 54, "a resize without a height is not half-applied");
  assert.equal(f.controller.view().barPosition, null, "and none of them moves the bar either");

  f.bar.handler!(JSON.stringify({
    action: "resized", sessionId: "session-fixed", x: 12.4, y: 40, width: 1_200.6, height: 95.6
  }));
  assert.deepEqual(f.controller.view().barPosition, { x: 12, y: 40 },
    "one resize saves the corner, the width and the height");
  assert.equal(f.bar.snapshots.at(-1)?.width, 1_201);
  assert.equal(f.bar.snapshots.at(-1)?.height, 96);
  assert.equal(f.bar.snapshots.at(-1)?.endsAtEpochSeconds, clockBefore,
    "resizing the bar never moves the clock it is counting down");
  ...
  assert.equal(f.bar.snapshots.at(-1)?.height, 96, "and the height");
  ...
  assert.equal(restarted.bar.snapshots.at(-1)?.height, 96, "and so is the height");
});
